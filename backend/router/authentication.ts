...
authenticationRouter.post("/signup", async (req, res) => {
  ...
  await client.xAdd(TRADE_STREAM, "*", {
    message: JSON.stringify({
      operation: Operations.UserRegister,
...
authenticationRouter.post(
  "/signin",
  async (
    req: Request<{}, any, { otp: string; email: string; auth_token: string }>,
    res
  ) => {
...
authenticationRouter.get(
  "/signin",
  (req: Request<any, any, any, { email: string; auth_token: string }>, res) => {
    // check auth_token
